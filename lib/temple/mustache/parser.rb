...
EOF
      ...
      def scan_tags
        ...
        when '#'
          block = [:multi]
          @result << [:mustache, :section, content, block, @scanner.pos + 2]
        ...
        when '/'
          ...
          if section[1] == :section
            section[4] = @scanner.string[section[4]...@scanner.pos - content.length - 3]
          end
        ...
        when '{', '&'
          ...
          # aesthetics.
          type = "}" if type == "{"
          @result << [:mustache, :tag, content, false]
        else
          @result << [:mustache, :tag, content, true]
        end
